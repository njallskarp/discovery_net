...
# ------------------------------------------------------------------- the node
NODES="$(ls "$HERE/bindings/nodes"/*.env 2>/dev/null || true)"
[ -n "$NODES" ] || die "no node bindings yet. Run agent-sessions/init-node.sh first."
...
while :; do
  ...
  NODE_FILE="$HERE/bindings/nodes/$NODE_NAME.env"
  [ -f "$NODE_FILE" ] && break
...
OUT="$HERE/bindings/agents/$AGENT.env"
write_env "$OUT" <<EOF
...
say ""
if "$HERE/run.sh" "$AGENT" --dry-run; then
  say ""
