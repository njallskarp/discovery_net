    ...
    *,
    ...
    if validator_governance is not None:
        ...
        expected_operators = tuple(
            sorted(
                (
                    (validator.public_key, validator.governance_public_key)
                    for validator in validators
                ),
        ...
        configured_operators = tuple(
            (
                operator.consensus_public_key,
                operator.governance_public_key,
            )
            for operator in validator_governance.operators
        )
        if any(key is None for _, key in expected_operators) or configured_operators != (
            expected_operators
        ):
            raise ValueError(
                "validator governance must map every genesis consensus key to its governance key"
            )
...
def _render_genesis(
    *,
    ...
    if validator_governance is not None:
        encoded_state = encode_genesis_application_state(validator_governance.initial_state())
